...
type CellType = 0 | 1 | 2 | 3 | 4 | 5 | 6 | 7 | 8  // 0: empty, 1: entrance, 2: exit, 3: fixed wall, 4: horizontal wall, 5: vertical wall, 6: Horizontal Empty, 7: Vertical Empty, 8: Hidden Treasure
type CellState = {
  type: CellType
  hit: boolean
  style: CellStyle
...
export default function SoloGame() {
  ...
  const generateMaze = (size: number, entrance: entrance, exit: exit, hiddenTreasure:hiddenTreasure, wall: number): [CellState[][], number] | null => {
  ...
  const generateValidMaze = (size: number, wall: number): [CellState[][], entrance, exit, hiddenTreasure, number] => {
    ...
    // Generate the maze
    const maze = generateMaze(size, entrance, exit, hiddenTreasure, wall)
    return maze ? [maze[0], entrance, exit, hiddenTreasure, maze[1]] : generateValidMaze(size, wall)
  }
    ...
    if (
        ...
        if (newPlayerX === hiddenTreasure.x && newPlayerY === hiddenTreasure.y) {
            if (!unlockTreasure) {
                ...
                // Also add 3 to the player's Life
  ...
  const startGame = (): void => {
      try {
          ...
          const [newMaze, entrance, exit, hiddenTreasure, steps] = generateValidMaze(mazeSize, totalWalls)
          if (newMaze) {
